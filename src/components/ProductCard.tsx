  ...
  return (
    ...
    >
      <Link to={`/product/${p.slug || p.id}`} className="block">
        <div className="relative overflow-hidden rounded-[2.5rem] bg-muted/50 aspect-[4/5] mb-5 shadow-sm group-hover:shadow-xl group-hover:shadow-primary/5 transition-all duration-500">
          <img
          ...
          {p.stock === 0 && (
            <div className="absolute inset-0 bg-black/40 flex items-center justify-center p-6 text-center">
               <div className="bg-background/90 backdrop-blur-md px-4 py-2 rounded-xl shadow-2xl border border-white/20 transform rotate-[-5deg]">
                  <p className="text-xs font-bold text-rose-500 uppercase tracking-widest leading-none">Sold Out</p>
                  <p className="text-[10px] text-muted-foreground font-medium mt-1">Check back later</p>
               </div>
            </div>
          )}
          ...
          <WishlistButton
            productId={p.id}
            productName={p.name}
            className="absolute top-4 right-4 w-10 h-10 bg-background/90 backdrop-blur-md shadow-md border-transparent"
          />
          ...
          {p.isBestSeller && !isRare && !isRisingStar && !hasMomentum && (
            <div className="absolute top-4 left-4 px-3 py-1 bg-primary text-primary-foreground text-[10px] font-bold uppercase tracking-wider rounded-full shadow-lg z-10">
              Best Seller
            </div>
          )}

          <div className={`absolute inset-x-4 bottom-4 translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300 ${p.stock === 0 ? "hidden" : ""}`}>
            <motion.button
              ...
              className="w-full flex items-center justify-center gap-2 py-3 rounded-2xl bg-background/95 backdrop-blur-xl text-foreground text-sm font-bold shadow-xl border border-white/20"
            >
              <ShoppingBag size={16} />
              Quick Add
            </motion.button>
      ...
      <div className="px-1">
        ...
        <div className="flex items-center justify-between mt-1">
          <p className="text-lg font-bold text-foreground">{p.priceFormatted}</p>
          {(p.stock || 0) < 10 && p.stock > 0 && (
            <motion.div 
               initial={{ scale: 0.9 }}
               animate={{ scale: 1 }}
